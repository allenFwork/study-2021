...
幸存区又分为From和To区。当Eden区的空间用完时，程序又需要创建对象，JVM的垃圾回收器将Eden区进行垃圾回收（Minor GC），将Eden区中的不再被其它对象应用的对象进行销毁。然后将Eden区中剩余的对象移到From 
...
![19](images\垃圾回收\19.png)![20](images\垃圾回收\20.png)
